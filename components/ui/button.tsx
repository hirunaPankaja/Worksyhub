    ...
    "inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 active:scale-95",
    {
        variants: {
            variant: {
                ...
                glass: "bg-white/20 backdrop-blur-sm border border-white/30 text-foreground hover:bg-white/30 dark:bg-black/20 dark:border-white/10 dark:hover:bg-black/30",
            },
            size: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
    ({ className, variant, size, asChild = false, ...props }, ref) => {
        // If we don't have Slot (radix), we'll simplify. Assume standard button for now.
        // Ideally we install @radix-ui/react-slot but I'll write a simple version if needed.
